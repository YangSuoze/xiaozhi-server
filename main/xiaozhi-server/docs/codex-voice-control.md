...
4. 用户可以发送指令、询问状态、切换任务或停止监控。
   也可以询问“之前做了什么”“还有什么没有解决”等任务历史问题。
5. 小智会先复述识别出的指令或回答。用户说“确认发送”后才进入任务队列；
   也可以说“修改为……”重新校正，或说“取消发送”。
6. 指令真正被 Codex 桌面应用接收后，小智才会播报发送成功。
7. 用户可以开启每 1 至 120 分钟一次的进度播报。

进度来自 Codex 桌面任务最近的过程说明和完成结果。桌面状态接口没有返回
正在执行的文本时，桥接只读取本机 Codex 记录中的助手过程消息，不读取推理、
用户消息或命令输出。快照包含“正在做”、“刚完成”和“下一步”三个可选字段。
桥接会删除代码块、路径、网址和疑似密钥，
再将短文本发送给服务器；没有可靠信息时会明确说最近没有新的步骤说明，
不会根据耗时猜测完成百分比。

服务器按任务保存最近 20 条经过上述过滤的 Codex 过程说明和最终回答，所有
消息正文合计最多 30,000 个字符。重复消息按消息 ID 覆盖，超过条数或字符上限时
先删除最旧记录。历史问答复用当前小智对话模型，只依据这些记录、当前状态和进度
生成简短回答；没有证据的内容会明确说明无法确定。
...
- 新指令和对 Codex 问题的回答都先保存为单条会话草稿，不会直接发给电脑。
- 修改草稿会覆盖旧内容；确认发送或取消后立即删除草稿。
